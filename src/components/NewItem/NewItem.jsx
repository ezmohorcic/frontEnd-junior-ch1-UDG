...
export default function NewItem()
{
  ...
  return(
      <div className={addShow ? "add" : "addBelow"} >
        <div id='addOut' onClick={handlerShowNew}> <p>X</p> </div>
        <div id='addCard'>
          {
            headers.map((e,i) => {
              return (
              i !== 3 ? <input key={"newInput-"+i} className={'addNewInput normalNewWidth'} value={newItem[e] ? newItem[e] : ""} type={e} name={e} placeholder={e} onChange={handleAddFormChange} />
              :
              <textarea key={"newInput-"+i} className={'addNewInput bigNewWidth'} value={newItem[e] ? newItem[e] : ""} type={e} name={e} placeholder={e} onChange={handleAddFormChange} />
            )})
          }     
        <button id='addSubmit' type="submit" onClick={handleSubmitNew}>Add</button> 
      </div> 
